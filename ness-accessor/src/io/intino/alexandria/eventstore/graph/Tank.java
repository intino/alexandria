...
import io.intino.alexandria.eventstore.MessageInputStreamBuilder;
import io.intino.alexandria.eventstore.Scale;
import io.intino.alexandria.eventstore.Sorter;
import io.intino.alexandria.eventstore.graph.AbstractTank;
import io.intino.alexandria.eventstore.graph.DatalakeGraph;
...
import static io.intino.alexandria.eventstore.AttachmentLoader.loadAttachments;
...
public class Tank extends AbstractTank {
	...
	private static Logger logger = LoggerFactory.getLogger(io.intino.alexandria.eventstore.graph.Tank.class);
	...
	public io.intino.alexandria.eventstore.graph.Tank init() {
		directory().mkdirs();
		return this;
	}
	...
	public class SortedMessagesIterator {
		public Iterator<Message> iterator(Instant from, Instant to) {
			try {
				File[] files = directory().listFiles((f, n) -> n.endsWith(io.intino.alexandria.eventstore.graph.Tank.INL) || n.endsWith(io.intino.alexandria.eventstore.graph.Tank.ZIP));
