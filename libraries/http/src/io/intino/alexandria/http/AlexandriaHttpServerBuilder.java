...
public class AlexandriaHttpServerBuilder {
	...
	private static AlexandriaHttpServer<?> getInstance() {
		return ui ? loadUIServer() : new AlexandriaHttpServer<>(port, webDirectory);
	}
	...
	private static AlexandriaHttpServer<?> loadUIServer() {
		try {
			...
			return (AlexandriaHttpServer<?>) constructor.newInstance(port, webDirectory, !objects.isEmpty() ? objects.get(0) : null);
