...
public class TransactionSession {
	...
	private <T extends Transaction> LedStream.Builder<T> builder(Fingerprint fingerprint, Class<T> transactionClass) {
		if (!builders.containsKey(fingerprint))
			builders.put(fingerprint, new UnsortedLedStreamBuilder<>(transactionClass, Transaction.factoryOf(transactionClass), transactionsBufferSize, provider.file(fingerprint.name(), Session.Type.led)));
