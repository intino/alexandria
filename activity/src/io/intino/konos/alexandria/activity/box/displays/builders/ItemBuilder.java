...
import io.intino.konos.alexandria.activity.box.model.mold.stamps.*;
import io.intino.konos.alexandria.activity.box.model.mold.stamps.icons.ResourceIcon;
import io.intino.konos.alexandria.activity.box.schemas.Item;
import io.intino.konos.alexandria.activity.box.schemas.ItemBlock;
import io.intino.konos.alexandria.activity.box.schemas.ItemStamp;
import io.intino.konos.alexandria.activity.box.schemas.Property;
...
import static io.intino.konos.alexandria.activity.box.Asset.toResource;
import static java.util.Collections.singletonList;
import static java.util.stream.Collectors.toList;

public class ItemBuilder {

    public static Item build(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
    ...
    public static List<Item> buildList(List<io.intino.konos.alexandria.activity.box.model.Item> itemList, ItemBuilderProvider provider, URL baseAssetUrl) {
        return itemList.stream().map(item -> ItemBuilder.build(item, provider, baseAssetUrl)).collect(toList());
    }

    private static String label(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider) {
    ...
    private static Instant group(io.intino.konos.alexandria.activity.box.model.Item item, TimeScale scale) {
        if (item == null) return null;
        Instant created = item.created();
        return created != null ? scale.normalise(created) : null;
    }

    private static List<ItemBlock> itemBlockList(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
        return provider.blocks().stream().map(block -> itemBlock(item, provider, baseAssetUrl, block)).collect(toList());
    }

    private static ItemBlock itemBlock(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl, Block block) {
        return new ItemBlock().name(block.name()).hidden(block.hidden(item));
    }

    private static List<ItemStamp> itemStampList(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
        return provider.stamps().stream().map(stamp -> recordItemStamp(item, provider, baseAssetUrl, stamp)).collect(toList());
    }

    private static ItemStamp recordItemStamp(io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl, Stamp stamp) {
    ...
    private static List<String> valuesOf(Stamp stamp, io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
    ...
    private static List<Property> propertiesOf(Stamp stamp, io.intino.konos.alexandria.activity.box.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
