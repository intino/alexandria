...
import io.intino.konos.alexandria.activity.box.displays.builders.DialogBuilder;
import io.intino.konos.alexandria.activity.box.displays.builders.ValidationBuilder;
...
public abstract class AlexandriaDialogDisplay extends AlexandriaDisplay<AlexandriaDialogDisplayNotifier> {
	...
	public io.intino.konos.alexandria.activity.box.spark.ActivityFile downloadResource(String path) {
		io.intino.konos.alexandria.activity.box.schemas.Resource resource = dialog.input(path).value().asResource();

		return new io.intino.konos.alexandria.activity.box.spark.ActivityFile() {
			@Override
			public String label() {
	...
	private DialogValidator.Result validateResource(FormInput formInput) {
		...
		List<io.intino.konos.alexandria.activity.box.schemas.Resource> resourceValues = dialog.input(formInput.path()).values().asResource();
		Map<String, byte[]> valuesMap = resourceValues.stream().collect(toMap(io.intino.konos.alexandria.activity.box.schemas.Resource::name, o -> Base64.decodeBase64(o.value())));
