...
import io.intino.konos.alexandria.activity.box.displays.notifiers.AlexandriaPanelCatalogViewDisplayNotifier;
import io.intino.konos.alexandria.activity.box.model.Catalog;
import io.intino.konos.alexandria.activity.box.model.TemporalCatalog;
...
public class AlexandriaPanelCatalogViewDisplay extends AlexandriaPanelViewDisplay<AlexandriaPanelCatalogViewDisplayNotifier> {
	...
	private void createCatalogDisplay() {
		...
		buildDisplay(catalog).ifPresent(display -> {
			display.filter(item -> render.filter(context(), target(), (io.intino.konos.alexandria.activity.box.model.Item) item));
