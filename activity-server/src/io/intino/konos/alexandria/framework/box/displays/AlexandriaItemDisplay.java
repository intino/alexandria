...
import io.intino.konos.alexandria.framework.box.displays.builders.MoldBuilder;
import io.intino.konos.alexandria.framework.box.displays.notifiers.AlexandriaItemDisplayNotifier;
import io.intino.konos.alexandria.framework.box.displays.providers.ItemDisplayProvider;
import io.intino.konos.alexandria.framework.box.helpers.ElementHelper;
...
import io.intino.konos.alexandria.framework.box.model.mold.stamps.CatalogLink;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.EmbeddedCatalog;
...
import io.intino.konos.alexandria.framework.box.model.mold.stamps.operations.DownloadOperation;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.operations.ExportOperation;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.pages.ExternalPage;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.pages.InternalPage;
...
public class AlexandriaItemDisplay extends ActivityDisplay<AlexandriaItemDisplayNotifier> {
	...
	private io.intino.konos.alexandria.framework.box.model.Item item;
	private String mode;
	...
	public void item(io.intino.konos.alexandria.framework.box.model.Item item) {
		this.item = item;
	}
	...
	public void itemStampsReady(String id) {
		...
		embeddedCatalogs().forEach((key, display) -> {
			display.filter(item -> key.filter(context, AlexandriaItemDisplay.this.item, (io.intino.konos.alexandria.framework.box.model.Item) item));
	...
	public void selectElement(Reference reference) {
		...
		display.filterAndNotify(item -> catalogLinkStamp.filter(this.item, (io.intino.konos.alexandria.framework.box.model.Item)item));
	...
	private Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> displays(io.intino.konos.alexandria.framework.box.model.Item item) {
		List<Stamp> stamps = provider.stamps(mold).stream().filter(s -> s instanceof io.intino.konos.alexandria.framework.box.model.mold.stamps.Display).collect(toList());
		Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> mapWithNulls = stamps.stream().collect(HashMap::new, (map, stamp)->map.put((io.intino.konos.alexandria.framework.box.model.mold.stamps.Display)stamp, provider.display(stamp.name())), HashMap::putAll);
		Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> result = mapWithNulls.entrySet().stream().filter(e -> e.getValue() != null).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
		result.forEach((key, value) -> value.item(item));
		return result;
	}

	private List<AlexandriaDisplay> pages(io.intino.konos.alexandria.framework.box.model.Item item) {
	...
	private PageLocation location(Stamp stamp, io.intino.konos.alexandria.framework.box.model.Item item) {
