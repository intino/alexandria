...
const ReelToolbarStyles = {
    label : { fontSize:'12pt',marginRight:'5px' },
    icon : { height:'20px',width:'20px' },
};

const ReelToolbar = ({ label, scales, toolbar, onChangeScale, onPrevious, onNext, onFirst, onLast, translate, classes }) => {
    ...
    return (
        <div className="layout horizontal start center flex">
            ...
            <div className="layout vertical"><Typography style={ReelToolbarStyles.label}>{toolbar.label}</Typography></div>
            <div className="layout horizontal">
                <IconButton disabled={!toolbar.canPrevious} onClick={handleFirst} size="small" style={{color:previousColor}}><FirstPage style={ReelToolbarStyles.icon}/></IconButton>
                <IconButton disabled={!toolbar.canPrevious} onClick={handlePrevious} size="small" style={{color:previousColor}}><NavigateBefore style={ReelToolbarStyles.icon}/></IconButton>
                <IconButton disabled={!toolbar.canNext} onClick={handleNext} size="small" style={{color:nextColor}}><NavigateNext style={ReelToolbarStyles.icon}/></IconButton>
                <IconButton disabled={!toolbar.canNext} onClick={handleLast} size="small" style={{color:nextColor}}><LastPage style={ReelToolbarStyles.icon}/></IconButton>
            </div>
        </div>
    );
};
export default ReelToolbar;