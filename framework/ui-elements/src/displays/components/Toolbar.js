...
class Toolbar extends AbstractToolbar {
	...
	render() {
		...
		return (<div className={classNames("layout horizontal center", classes.toolbar)}>{React.Children.map(this.props.children, (child, i) => { return this.renderItem(child); })}</div>);
	};
