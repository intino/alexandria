...
public class JMXServerTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "operation")), (condition("trigger", "signature"))).add(expression().add(mark("returnType")).or(expression().add(literal("void")))).add(literal(" execute")).add(mark("name", "firstUpperCase")).add(literal("(")).add(mark("parameter").multiple(", ")).add(literal(");")),
			rule().add((condition("type", "jmx")), (condition("type", "implementation"))).add(literal("package ")).add(mark("package", "validname")).add(literal(".jmx;\n\nimport tara.magritte.Graph;\n\npublic class ")).add(mark("name", "firstUpperCase")).add(literal(" implements ")).add(mark("name", "firstUpperCase")).add(literal("JMX {\n\n    private final Graph graph;\n\n    public ")).add(mark("name", "firstUpperCase")).add(literal("(Graph graph) {\n        this.graph = graph;\n    }\n\n    ")).add(mark("operation", "implementation").multiple("\n\n")).add(literal("\n}")),
			rule().add((condition("type", "operation")), (condition("trigger", "implementation"))).add(literal("public ")).add(expression().add(mark("returnType")).or(expression().add(literal("void")))).add(literal(" execute")).add(mark("name", "firstUpperCase")).add(literal("(")).add(mark("parameter").multiple(", ")).add(literal(") {\n    ")).add(expression().add(mark("returnType", "return")).add(literal(" "))).add(literal("new ")).add(mark("package", "validname")).add(literal(".actions.")).add(mark("action", "firstUpperCase")).add(literal("Action(graph).execute(")).add(mark("parameter", "parameterName").multiple(", ")).add(literal(");\n}")),
