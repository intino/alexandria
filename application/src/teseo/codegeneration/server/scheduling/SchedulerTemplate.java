...
public class SchedulerTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "schedule")), not(condition("type", "onStart")), (condition("trigger", "create"))).add(literal("public static void ")).add(mark("name", "SnakeCaseToCamelCase", "firstLowerCase")).add(literal("(TeseoScheduler scheduler, Map<String, Object> parameters) {\n\ttry {\n\t\tJobDetail job = newJob(")).add(mark("name", "SnakeCaseToCamelCase")).add(literal("Trigger.class).withIdentity(\"")).add(mark("name")).add(literal("\").build();\n\t\tfor (Map.Entry<String, Object> entry : parameters.entrySet())\n\t\t\tjob.getJobDataMap().put(entry.getKey(), entry.getValue());\n\t\tscheduler.scheduleJob(job, newSet(")).add(mark("trigger").multiple(", ")).add(literal("), true);\n\t} catch (SchedulerException e) {\n\t\te.printStackTrace();\n\t}\n}")),
			rule().add((condition("type", "schedule")), not(condition("type", "onStart")), (condition("trigger", "switch"))).add(literal("if (")).add(mark("name", "SnakeCaseToCamelCase")).add(literal("Trigger.class.equals(triggerClass)) ")).add(mark("name", "SnakeCaseToCamelCase", "firstLowerCase")).add(literal("(scheduler, parameters);")),
			rule().add((condition("type", "schedule & onStart")), (condition("trigger", "init"))).add(literal("job = newJob(")).add(mark("name", "SnakeCaseToCamelCase")).add(literal("Trigger.class).withIdentity(\"")).add(mark("name")).add(literal("\").build();\njob.getJobDataMap().put(\"graph\", graph);\nscheduler.scheduleJob(job, newSet(")).add(mark("trigger").multiple(", ")).add(literal("), true);")),
			...
			rule().add((condition("type", "oneTimeTrigger")), (condition("trigger", "trigger"))).add(literal("newTrigger().startNow().build()"))
		);
		return this;
	}
}