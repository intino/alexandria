...
public class PassiveViewRequesterTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((type("parameter")), (trigger("request"))).output(mark("name")).output(literal(" = function(value) {\n    if (this.addToHistory(value)) return;\n\t")).output(mark("method")).output(literal("({ op: \"")).output(mark("name")).output(literal("\", s: \"")).output(mark("display", "lowercase")).output(literal("\", d: this.element.shortId(), o: this.element.props.owner(), c: this.element.props.context(), v: ")).output(mark("parameter")).output(literal("}, this.element.ownerUnit());\n}")),
			rule().condition((trigger("request"))).output(mark("name")).output(literal(" = (")).output(expression().output(mark("parameterSignature"))).output(literal(") => {\n    if (this.addToHistory(")).output(expression().output(mark("parameterSignature"))).output(literal(")) return;\n    ")).output(mark("method")).output(literal("({ op: \"")).output(mark("name")).output(literal("\", s: \"")).output(mark("display", "lowercase")).output(literal("\", d: this.element.shortId(), o: this.element.props.owner(), c: this.element.props.context()")).output(expression().output(literal(", v: ")).output(mark("parameter"))).output(literal("}, this.element.ownerUnit());\n};")),
			rule().condition((type("object")), (trigger("parameter"))).output(literal("encodeURIComponent(JSON.stringify(value))")),
			rule().condition((allTypes("list","file")), (trigger("parameter"))).output(literal("value")),
