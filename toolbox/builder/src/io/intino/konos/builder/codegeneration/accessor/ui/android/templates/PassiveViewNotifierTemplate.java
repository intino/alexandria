...
public class PassiveViewNotifierTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((allTypes("parameter","datetime"))).output(literal("kotlinx.datetime.Instant.fromEpochMilliseconds(it[\"v\"]!!.toLong())")),
			rule().condition((allTypes("parameter","date"))).output(literal("kotlinx.datetime.Instant.fromEpochMilliseconds(it[\"v\"]!!.toLong())")),
			rule().condition((allTypes("parameter","real"))).output(literal("it[\"v\"]!!.toDouble()")),
			rule().condition((allTypes("parameter","longinteger"))).output(literal("it[\"v\"]!!.toLong()")),
			rule().condition((allTypes("parameter","integer"))).output(literal("it[\"v\"]!!.toInt()")),
			rule().condition((allTypes("parameter","boolean"))).output(literal("it[\"v\"]!!.toBoolean()")),
			rule().condition((allTypes("parameter","object"))).output(literal("io.intino.alexandria.mobile.util.Json.parse(it[\"v\"]!!) as ")).output(mark("package")).output(literal(".mobile.schemas.")).output(mark("value", "firstUpperCase")),
			rule().condition((type("parameter"))).output(literal("it[\"v\"]!!")),
			rule().condition((attribute("", "Display")), (trigger("target"))).output(literal(".toSelf()")),
			rule().condition((trigger("target"))),
			rule().condition((type("event"))).output(literal("onMessage(\"")).output(mark("name", "firstLowerCase")).output(literal("\").toSelf().execute { element.")).output(mark("name", "firstLowerCase")).output(literal("(it")).output(expression().output(literal("\"v\""))).output(literal("!!) }"))
		);
	}
}