...
public class RouteDispatcherTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((allTypes("resource","main")), (trigger("call"))).output(literal("if (address.length() <= 1) { dispatch")).output(mark("name", "firstUpperCase")).output(literal("(soul); return; }")),
