...
public class MounterTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((trigger("switch"))).output(literal("if\t(event.toMessage().type().equals(\"")).output(mark("")).output(literal("\")) handle((")).output(mark("")).output(literal(") event);")),
			rule().condition((trigger("method"))).output(literal("public void handle(")).output(mark("")).output(literal(" event) {\n\n}")),
