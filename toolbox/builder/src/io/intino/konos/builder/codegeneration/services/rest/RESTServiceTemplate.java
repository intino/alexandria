...
public class RESTServiceTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((trigger("hasnotifications"))).output(literal("if (!io.intino.alexandria.http.AlexandriaSparkBuilder.isUI()) server.route(\"/push\").push(new SparkPushService());"))
		);
	}
}