...
public class AdapterRenderer {
	...
	public void execute() {
		for (Adapter adapter : adapters) {
			...
			final File directory = new File(settings.src(Target.Owner), "datahub/adapters");
			final String adapterName = adapter.name$();
			settings.classes().put(adapter.getClass().getSimpleName() + "#" + adapter.name$(), "datahub.adapters." + adapterName);
