...
public class MainTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
				...
				rule().condition((type("model")), (trigger("model"))).output(literal("io.intino.magritte.framework.Graph graph = new io.intino.magritte.framework.Graph().loadStashes(\"")).output(mark("name")).output(literal("\");"))
		);
	}
}