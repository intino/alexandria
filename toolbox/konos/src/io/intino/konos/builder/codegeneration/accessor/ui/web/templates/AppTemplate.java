...
public class AppTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(allTypes("page"), trigger("render"))).output(literal("const requireGoogleApi = Application.configuration.googleApiKey != null && Application.configuration.googleApiKey !== \"\";\nconst content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><><")).output(placeholder("templateName", "firstUpperCase")).output(literal("Page/><ReadyGate/></></GoogleApiLoader> : <><")).output(placeholder("templateName", "firstUpperCase")).output(literal("Page/><ReadyGate/></>;\nconst ")).output(placeholder("templateName", "firstLowerCase")).output(literal(" = document.getElementById(\"")).output(placeholder("templateName", "firstUpperCase")).output(literal("\");\nif (")).output(placeholder("templateName", "firstLowerCase")).output(literal(") createRoot(")).output(placeholder("templateName", "firstLowerCase")).output(literal(").render(<DisplayRouter id=\"__router__\" owner={()=>\"\"} context={()=>\"\"}>{content}</DisplayRouter>);\nif (!requireGoogleApi) openPushService();")));
		return rules;
	}
