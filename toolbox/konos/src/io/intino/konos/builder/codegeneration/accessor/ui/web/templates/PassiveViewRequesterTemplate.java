...
public class PassiveViewRequesterTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(allTypes("parameter"), trigger("request"))).output(placeholder("name")).output(literal(" = function(value")).output(expression().output(literal(", ")).output(placeholder("progressCallback"))).output(literal(") {\n\tif (this.addToHistory(value)) return;\n\t")).output(placeholder("method")).output(literal("({ app: this.element.context, op: \"")).output(placeholder("name")).output(literal("\", s: \"")).output(placeholder("display", "lowercase")).output(literal("\", d: this.element.shortId(), o: this.element.props.owner(), c: this.element.props.context(), v: ")).output(placeholder("parameter")).output(literal("}, this.element.ownerUnit()")).output(expression().output(literal(", undefined, ")).output(placeholder("progressCallback"))).output(literal(");\n}")));
		rules.add(rule().condition(trigger("request")).output(placeholder("name")).output(literal(" = (")).output(expression().output(placeholder("parameterSignature"))).output(expression().output(literal(", ")).output(placeholder("progressCallback"))).output(literal(") => {\n\tif (this.addToHistory(")).output(expression().output(placeholder("parameterSignature"))).output(literal(")) return;\n\t")).output(placeholder("method")).output(literal("({ app: this.element.context, op: \"")).output(placeholder("name")).output(literal("\", s: \"")).output(placeholder("display", "lowercase")).output(literal("\", d: this.element.shortId(), o: this.element.props.owner(), c: this.element.props.context()")).output(expression().output(literal(", v: ")).output(placeholder("parameter"))).output(literal("}, this.element.ownerUnit()")).output(expression().output(literal(", undefined, ")).output(placeholder("progressCallback"))).output(literal(");\n};")));
