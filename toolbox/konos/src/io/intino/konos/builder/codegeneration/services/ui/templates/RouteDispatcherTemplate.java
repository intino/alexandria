...
public class RouteDispatcherTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(allTypes("resource", "main"), trigger("pattern"))).output(literal("if (addressPart.equals(\"/\") || addressPart.matches(patterns.get(\"")).output(placeholder("name")).output(literal("\"))) return patterns.get(\"")).output(placeholder("name")).output(literal("\");")));
		rules.add(rule().condition(all(allTypes("resource"), trigger("pattern"))).output(literal("else if (addressPart.matches(patterns.get(\"")).output(placeholder("name")).output(literal("\"))) return patterns.get(\"")).output(placeholder("name")).output(literal("\");")));
