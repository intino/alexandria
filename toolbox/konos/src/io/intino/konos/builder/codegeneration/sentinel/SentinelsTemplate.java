...
public class SentinelsTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(all(allTypes("sentinel"), allTypes("webhook")), trigger("actioncall"))).output(literal("private static ")).output(placeholder("name", "PascalCase")).output(literal("Action ")).output(placeholder("name", "CamelCase")).output(literal("Action(io.intino.alexandria.http.server.AlexandriaHttpManager manager) {\n\t")).output(placeholder("name", "PascalCase")).output(literal("Action action = new ")).output(placeholder("name", "PascalCase")).output(literal("Action();\n\t")).output(placeholder("parameter", "assign").multiple("\n")).output(literal("\n\treturn action;\n}")));
