...
public class ViewSrcTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "view & container & mold"))),
			rule().add((condition("type", "view & container & catalog"))).add(literal("public static class ")).add(mark("name", "firstUpperCase")).add(literal(" {\n\t")).add(mark("filter")).add(literal("\n\n\t")).add(mark("hidden")).add(literal("\n}")),
			rule().add((condition("type", "view & container & set"))).add(mark("item").multiple("\n")),
