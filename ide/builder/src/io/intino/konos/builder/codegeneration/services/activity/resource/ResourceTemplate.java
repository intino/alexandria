...
public class ResourceTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "parameter"))).add(literal("action.")).add(mark("name", "SnakeCaseToCamelCase", "firstLowerCase")).add(literal(" = !manager.fromPath(\"")).add(mark("name")).add(literal("\", String.class).isEmpty() ? new String(Base64.getDecoder().decode(manager.fromPath(\"")).add(mark("name")).add(literal("\", String.class))) : \"\";"))
		);
		return this;
	}
}