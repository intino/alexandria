...
public class PanelTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "filter"))).add(literal("public static boolean filter(")).add(mark("box")).add(literal("Box box, io.intino.konos.alexandria.activity.model.Catalog catalog, io.intino.konos.alexandria.activity.model.Element context, Object target, Object object, ActivitySession session) {\n\treturn true;//TODO\n}")),
			rule().add((condition("trigger", "hidden"))).add(literal("public static boolean hidden(")).add(mark("box")).add(literal("Box box, Object object, ActivitySession session) {\n\treturn false;//TODO\n}")),
