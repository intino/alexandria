...
public class ThemeTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "typography"))).add(literal("fontFamily : '")).add(mark("fontFamily")).add(literal("',\nfontSize : ")).add(mark("fontSize")).add(literal(",\nuseNextVariants: true")),
			rule().add((condition("type", "format"))).add(mark("name")).add(literal(": { ")).add(mark("content")).add(literal(" }"))
		);
		return this;
	}
}