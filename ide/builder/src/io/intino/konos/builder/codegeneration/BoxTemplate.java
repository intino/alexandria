...
public class BoxTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("trigger", "rest"))).add(literal("protected io.intino.konos.alexandria.ui.services.AuthService authService(java.net.URL authServiceUrl) {\n\t//TODO add your authService\n\treturn null;\n}\t\n\nprotected io.intino.konos.alexandria.ui.services.EditorService editorService(java.net.URL editorServiceUrl) {\n\t//TODO add your editorService\n\treturn null;\n}")),
			rule().add((condition("trigger", "authenticationValidator"))).add(literal("public io.intino.konos.alexandria.rest.security.")).add(mark("type", "FirstUpperCase")).add(literal("AuthenticationValidator authenticationValidator() {\n\treturn token -> false;\n}"))
		);
		return this;
	}
}