...
public class NessTanksTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "tank")), (condition("trigger", "subscribe"))).add(literal("new TopicConsumer(box.datalake().session(), ")).add(mark("name")).add(literal(".flowChannel()).listen(new ")).add(mark("name", "snakeCaseToCamelCase", "FirstUpperCase")).add(literal("Subscriptor(box), box.configuration().nessConfiguration().clientID + \"-")).add(mark("name")).add(literal("\");")),
			...
			rule().add((condition("trigger", "tankImport"))).add(literal("import ")).add(mark("value", "validPackage")).add(literal(".messagehandlers.*;"))
		);
		return this;
	}
}