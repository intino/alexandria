...
public class LayoutTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "layout"))).add(literal("package ")).add(mark("package")).add(literal(".displays;\n\nimport ")).add(mark("package", "validPackage")).add(literal(".")).add(mark("box", "firstUpperCase")).add(literal("Box;\n\npublic class ")).add(mark("name", "FirstUpperCase")).add(literal("Layout extends Abstract")).add(mark("name", "FirstUpperCase")).add(literal("Layout {\n\n\tpublic ")).add(mark("name", "FirstUpperCase")).add(literal("Layout(")).add(mark("box", "firstUpperCase")).add(literal("Box box) {\n\t\tsuper(box);\n\t}\n\n\t@Override\n\tprotected void init() {\n\t\tsuper.init();\n\t}\n}"))
		);
		return this;
	}
}