...
public class LayoutTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "catalogs"))).add(expression().add(mark("filtered")).add(literal("public static boolean filter(io.intino.konos.alexandria.activity.model.Catalog catalog, io.intino.konos.alexandria.activity.model.Element context, Object target, Object object, User user) {")).add(literal("\n")).add(literal("\treturn true;")).add(literal("\n")).add(literal("}"))),
			rule().add((condition("type", "objects"))).add(literal("public static java.util.List<io.intino.konos.alexandria.activity.model.renders.RenderObjects.Source.Entry> objects(")).add(mark("box", "firstUpperCase")).add(literal("Box box, User user) {\n\treturn java.util.Collections.emptyList();\n}")),
