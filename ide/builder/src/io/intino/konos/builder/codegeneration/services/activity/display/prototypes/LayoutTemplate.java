...
public class LayoutTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "catalogs"))).add(expression().add(mark("filtered")).add(literal("public static boolean filter(io.intino.konos.alexandria.activity.model.Catalog catalog, io.intino.konos.alexandria.activity.model.Element context, Object target, Object object, ActivitySession session) {")).add(literal("\n")).add(literal("\treturn true;")).add(literal("\n")).add(literal("}"))),
			rule().add((condition("type", "objects"))).add(literal("public static java.util.List<io.intino.konos.alexandria.activity.model.renders.RenderObjects.Source.Entry> objects(")).add(mark("box", "firstUpperCase")).add(literal("Box box, ActivitySession session) {\n\treturn java.util.Collections.emptyList();\n}")),
			rule().add((condition("type", "display"))).add(literal("public static AlexandriaDisplay ")).add(mark("display", "firstLowerCase")).add(literal("(")).add(mark("box", "firstUpperCase")).add(literal("Box box, Object target, Consumer<Boolean> loadingListener, Consumer<CatalogInstantBlock> instantListener) {\n\treturn null;//TODO\n}")),
			rule().add((condition("type", "panels"))),
			rule().add((condition("trigger", "quoted"))).add(literal("\"")).add(mark("value")).add(literal("\"")),
			rule().add((condition("trigger", "hidden"))).add(literal("public static boolean hidden(")).add(mark("box", "firstUpperCase")).add(literal("Box box, ActivitySession session) {\n\treturn false;//TODO\n}"))
		);
		return this;
	}
}