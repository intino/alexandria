...
public class DisplayRequesterTemplate extends Template {
	...
	public Template define() {
		add(
			rule().add((condition("type", "display"))).add(literal("package ")).add(mark("package", "validPackage")).add(literal(".displays.requesters;\n\nimport ")).add(mark("package", "validPackage")).add(literal(".displays.")).add(mark("name", "firstUpperCase")).add(mark("type", "format", "FirstUpperCase")).add(literal(";\nimport io.intino.konos.alexandria.exceptions.*;\nimport ")).add(mark("package", "validPackage")).add(literal(".*;\n")).add(mark("schemaImport")).add(literal("\n\nimport ")).add(mark("package", "validPackage")).add(literal(".displays.notifiers.")).add(mark("name", "firstUpperCase")).add(literal("DisplayNotifier;\nimport io.intino.konos.alexandria.activity.displays.requesters.Alexandria")).add(expression().add(mark("type", "class", "FirstUpperCase"))).add(literal("DisplayRequester;\nimport io.intino.konos.alexandria.exceptions.AlexandriaException;\nimport io.intino.konos.alexandria.activity.displays.AlexandriaDisplayNotifierProvider;\nimport io.intino.konos.alexandria.activity.spark.ActivitySparkManager;\n\npublic class ")).add(mark("name", "firstUpperCase")).add(literal("DisplayRequester extends ")).add(expression().add(literal("Alexandria")).add(mark("type", "class", "FirstUpperCase"))).add(literal("DisplayRequester {\n\n\tpublic ")).add(mark("name", "firstUpperCase")).add(literal("DisplayRequester(ActivitySparkManager manager, AlexandriaDisplayNotifierProvider notifierProvider) {\n\t\tsuper(manager, notifierProvider);\n\t}\n\n\t@Override\n\tpublic void execute() throws AlexandriaException {\n\t\t")).add(mark("name", "firstUpperCase")).add(mark("type", "format", "FirstUpperCase")).add(literal(" display = display();\n\t\tif (display == null) return;\n\t\tString operation = operation();\n\n\t\t")).add(mark("request").multiple("\nelse ")).add(literal("\n\t\tsuper.execute();\n\t}\n}")),
			rule().add((condition("attribute", "Display")), (condition("trigger", "class"))),
			rule().add((condition("attribute", "MenuLayout")), (condition("trigger", "format"))).add(literal("Layout")),
			rule().add((condition("attribute", "TabLayout")), (condition("trigger", "format"))).add(literal("Layout")),
			rule().add((condition("attribute", "Display")), (condition("trigger", "format"))).add(literal("Display")),
