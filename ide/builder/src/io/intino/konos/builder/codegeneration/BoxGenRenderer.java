...
public class BoxGenRenderer {
	...
	public BoxGenRenderer(KonosGraph graph, File gen, String packageName, Module module, String parent, boolean hasModel) {
		konos = graph;
		this.gen = gen;
	...
	private void activities(Frame frame, String name) {
		List<UIService> uiServiceList = konos.uIServiceList();
		if (!uiServiceList.isEmpty()) {
			final Frame serviceFrame = new Frame();
			if (parent != null) serviceFrame.addSlot("parent", parent);
			frame.addSlot("hasUi", serviceFrame);
		}

		for (UIService service : uiServiceList)
			frame.addSlot("ui", (Frame) uiServiceFrame(service, name));
	}
	...
	private Frame uiServiceFrame(UIService service, String name) {
		...
		frame.addTypes("ui").addSlot("name", service.name$()).addSlot("configuration", name);
		return frame;
	}
	...
	private Template template() {
		return Formatters.customize(BoxGenTemplate.create());
	}
}
