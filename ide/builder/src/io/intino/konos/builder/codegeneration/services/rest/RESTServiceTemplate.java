...
public class RESTServiceTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "resource"))).add(literal("server.route(")).add(mark("path", "format")).add(literal(").")).add(mark("method", "firstlowerCase")).add(literal("(manager -> new ")).add(mark("name", "firstUpperCase", "SnakeCaseToCamelCase")).add(literal("Notification(box, manager).execute());")),
			...
			rule().add((condition("trigger", "notification"))).add(literal("server.route(\"/")).add(mark("path", "format")).add(literal("\").post(manager -> new Post")).add(mark("name", "firstUpperCase", "SnakeCaseToCamelCase")).add(literal("Resource(box, manager).execute());")),
			rule().add((condition("trigger", "hasNotifications"))).add(literal("server.route(\"/push\").push(new SparkPushService()); //solo una vez, con ui o con rest+notifications"))
		);
		return this;
	}
}