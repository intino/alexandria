...
public class ActivityTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "dialog"))).add(literal("spark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").before(manager -> new BeforeDisplayRequest(manager).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").post(manager -> new io.intino.konos.server.activity.dialogs.DialogRequester(manager, notifierProvider()).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").get(manager -> new io.intino.konos.server.activity.dialogs.DialogRequester(manager, notifierProvider()).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").after(manager -> new AfterDisplayRequest(manager).execute());")),
