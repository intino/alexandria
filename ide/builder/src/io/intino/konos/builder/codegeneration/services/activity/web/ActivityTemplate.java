...
public class ActivityTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "dialog")), (condition("trigger", "notifier"))).add(literal("register(io.intino.konos.alexandria.activity.displays.dialogs.DialogNotifier.class).forDisplay(")).add(mark("name", "firstUpperCase")).add(literal("Dialog.class);")),
			...
			rule().add((condition("type", "dialog"))).add(literal("spark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").before(manager -> new BeforeDisplayRequest(manager).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").post(manager -> new io.intino.konos.alexandria.activity.displays.dialogs.DialogRequester(manager, notifierProvider()).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").get(manager -> new io.intino.konos.alexandria.activity.displays.dialogs.DialogRequester(manager, notifierProvider()).execute());\nspark.route(\"/")).add(mark("name", "lowercase")).add(literal("dialog/:displayId\").after(manager -> new AfterDisplayRequest(manager).execute());")),
