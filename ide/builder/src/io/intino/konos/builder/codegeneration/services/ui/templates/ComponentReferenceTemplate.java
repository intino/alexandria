...
public class ComponentReferenceTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "input & filevalue"))).add(literal("public void onChange(io.intino.alexandria.ui.displays.events.ChangeListener listener) { this.changeListener = listener; }")),
			rule().add((condition("type", "input"))).add(literal("public void onChange(io.intino.alexandria.ui.displays.events.ChangeListener listener) { this.changeListener = listener; }\npublic void onKeyPress(io.intino.alexandria.ui.displays.events.KeyPressListener listener) { this.keyPressListener = listener; }"))
		);
		return this;
	}
}