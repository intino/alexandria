...
public class ViewTemplate extends Template {
	...
	public Template define() {
		add(
			rule().add((condition("type", "view & display"))).add(literal("public static class ")).add(mark("name", "firstUpperCase")).add(literal(" {\n\t")).add(expression().add(mark("catalogScope")).add(literal("\n")).add(literal("\t"))).add(literal("\n\tpublic static AlexandriaDisplay display(")).add(mark("box", "firstUpperCase")).add(literal("Box box, io.intino.konos.alexandria.ui.model.Element context, Consumer<Boolean> loadingListener, Consumer<io.intino.konos.alexandria.ui.displays.CatalogInstantBlock> instantListener, UISession session) {\n\t\treturn null;//TODO\n\t}\n\n\t")).add(mark("hidden")).add(literal("\n}")),
