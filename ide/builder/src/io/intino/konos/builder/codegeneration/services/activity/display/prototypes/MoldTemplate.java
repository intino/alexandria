...
public class MoldTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "stamp & location"))).add(literal("new Location().")).add(mark("common")).add(expression().add(literal(".")).add(mark("icon"))).add(literal(".name(\"s1\").label(\"coordinates\").value((object, username) -> TestInfrastructureMold.Stamps.coordinates((")).add(mark("moldType")).add(literal(") object, username)))")),
			...
			rule().add((condition("type", "stamp")), (condition("trigger", "class")))
		);
		return this;
	}
}