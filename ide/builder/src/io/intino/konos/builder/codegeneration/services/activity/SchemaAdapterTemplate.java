...
public class SchemaAdapterTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "multiple & member")), (condition("trigger", "componentfromLayer"))).add(literal("if (component.name().equalsIgnoreCase(")).add(expression().add(mark("package")).add(literal("."))).add(mark("type", "FirstUpperCase")).add(literal(".class.getSimpleName())) schema.")).add(mark("name", "firstLowerCase")).add(literal("List().add(")).add(mark("type", "FirstLowerCase")).add(literal("FromNode(component));"))
		);
		return this;
	}
}