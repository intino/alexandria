...
public class ChannelTemplate extends Template {
	...
	public Template define() {
		add(
				...
				rule().add((condition("type", "subscription"))).add(literal("new ")).add(mark("type")).add(literal("Consumer(session, \"")).add(mark("path")).add(literal("\").listen(new ")).add(mark("name", "firstUpperCase")).add(literal("Subscription(box)")).add(expression().add(literal(", ")).add(mark("durable"))).add(literal(");")),
				rule().add((condition("trigger", "durable"))).add(literal("box.configuration().")).add(mark("value", "firstLowerCase")).add(literal("Configuration.clientID")),
