...
public class RESTServiceTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "resource"))).add(literal("server.route(\"")).add(mark("path")).add(literal("\").")).add(mark("method", "firstlowerCase")).add(literal("((manager) -> new ")).add(mark("operation", "firstUpperCase")).add(mark("name", "firstUpperCase")).add(literal("Resource(graph, manager).execute());"))
		);
		return this;
	}
}