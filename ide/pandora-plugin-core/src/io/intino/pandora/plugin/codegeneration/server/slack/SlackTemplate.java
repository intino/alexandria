...
public class SlackTemplate extends Template {
	...
	public Template define() {
		add(
				rule().add((condition("type", "slack"))).add(literal("package ")).add(mark("package", "ValidPackage")).add(literal(";\n\nimport tara.magritte.Graph;\nimport io.intino.pandora.slack.Bot;\n\npublic class ")).add(mark("name", "firstUpperCase", "snakeCaseToCamelCase")).add(literal("SlackBot extends Bot {\n\n\tpublic ")).add(mark("name", "firstUpperCase", "snakeCaseToCamelCase")).add(literal("SlackBot(Graph graph) {\n\t\tsuper(\"")).add(mark("token")).add(literal("\");\n\t\t")).add(mark("request", "add").multiple("\n")).add(literal("\n\t}\n\t")).add(expression().add(literal("\n")).add(literal("\tpublic void notify(String message) {")).add(literal("\n")).add(literal("\t\tsend(\"")).add(mark("channel")).add(literal("\", message);")).add(literal("\n")).add(literal("\t}"))).add(literal("\n\n\t")).add(mark("request").multiple("\n\n")).add(literal("\n}")),
				rule().add((condition("type", "request")), (condition("trigger", "add"))).add(literal("add(\"")).add(mark("name")).add(literal("\", \"")).add(mark("description")).add(literal("\", (properties, args) -> ")).add(mark("name", "snakeCaseToCamelCase", "firstLowerCase")).add(literal("(graph, properties")).add(expression().add(literal(", ")).add(mark("parameter", "cast").multiple(", "))).add(literal("));")),
				rule().add((condition("type", "request"))).add(literal("private String ")).add(mark("name", "snakeCaseToCamelCase", "firstLowerCase")).add(literal("(Graph graph, MessageProperties properties")).add(expression().add(literal(", ")).add(mark("parameter").multiple(", "))).add(literal(") {\n\treturn \"\";\n}")),
