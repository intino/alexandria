...
import io.intino.konos.server.activity.displays.catalogs.builders.CatalogSortingBuilder;
import io.intino.konos.server.activity.displays.catalogs.model.Catalog;
import io.intino.konos.server.activity.displays.catalogs.model.events.OpenPanel;
...
import io.intino.konos.server.activity.displays.molds.builders.PictureDataBuilder;
...
public class CatalogListViewDisplay extends PageDisplay<CatalogListViewDisplayNotifier> implements CatalogViewDisplay {
    ...
    public CatalogListViewDisplay(Box box) {
        super(box);
    }
    ...
    private void renderDisplays(String item) {
        Map<io.intino.konos.server.activity.displays.molds.model.stamps.Display, StampDisplay> displays = displays(item);
    ...
    private void refreshPictures(String itemId, List<Picture> pictures) {
        io.intino.konos.server.activity.displays.elements.model.Item item = provider.item(new String(Base64.getDecoder().decode(itemId)));
    ...
    protected void init() {
        ...
        List<io.intino.konos.server.activity.displays.catalogs.model.arrangement.Sorting> sortings = provider.sortings();
    ...
    private void sendSortingList(List<io.intino.konos.server.activity.displays.catalogs.model.arrangement.Sorting> sortings) {
    ...
    private Map<io.intino.konos.server.activity.displays.molds.model.stamps.Display, StampDisplay> displays(String record) {
        List<Stamp> stamps = provider.stamps(view.mold()).stream().filter(s -> (s instanceof io.intino.konos.server.activity.displays.molds.model.stamps.Display)).collect(toList());
        Map<io.intino.konos.server.activity.displays.molds.model.stamps.Display, StampDisplay> nullableMap = stamps.stream().collect(Collectors.toMap(s -> (io.intino.konos.server.activity.displays.molds.model.stamps.Display)s, stamp -> provider.display(stamp.name())));
        Map<io.intino.konos.server.activity.displays.molds.model.stamps.Display, StampDisplay> result = nullableMap.entrySet().stream().filter(e -> e.getValue() != null).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
    ...
    private ElementViewDisplayProvider.Sorting sortingOf(io.intino.konos.server.activity.displays.catalogs.model.arrangement.Sorting sorting) {
    ...
    private ElementViewDisplayProvider.Sorting sortingOf(String name, String mode) {
        return new ElementViewDisplayProvider.Sorting() {
            ...
            @Override
            public int comparator(io.intino.konos.server.activity.displays.elements.model.Item item1, io.intino.konos.server.activity.displays.elements.model.Item item2) {
    ...
    private List<io.intino.konos.server.activity.displays.elements.model.Item> selectedItems() {
