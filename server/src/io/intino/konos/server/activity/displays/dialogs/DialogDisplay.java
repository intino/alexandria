...
import io.intino.konos.server.activity.displays.dialogs.model.Dialog.Tab.*;
import io.intino.konos.server.activity.displays.dialogs.model.Dialog.Tab.Date;
import io.intino.konos.server.activity.displays.dialogs.DialogValidator.Result;
import io.intino.konos.server.activity.displays.dialogs.builders.DialogBuilder;
import io.intino.konos.server.activity.displays.dialogs.builders.ValidationBuilder;
...
public abstract class DialogDisplay extends Display<DialogNotifier> {
	...
	public io.intino.konos.server.activity.spark.ActivityFile downloadResource(String path) {
		io.intino.konos.server.activity.displays.schemas.Resource resource = dialog.input(path).value().asResource();
	...
	private Result validateResource(FormInput formInput) {
		...
		List<io.intino.konos.server.activity.displays.schemas.Resource> resourceValues = dialog.input(formInput.path()).values().asResource();
		Map<String, byte[]> valuesMap = resourceValues.stream().collect(toMap(io.intino.konos.server.activity.displays.schemas.Resource::name, o -> Base64.decodeBase64(o.value())));
