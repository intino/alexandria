...
public class DialogBuilder {
    ...
    public static JsonObject jsonToolbar(io.intino.konos.server.activity.displays.dialogs.model.Dialog.Toolbar toolbar) {
        JsonObject result = new JsonObject();
        result.add("operationList", jsonOperationList(toolbar.operationList()));
        return result;
    }

    public static JsonArray jsonOperationList(List<io.intino.konos.server.activity.displays.dialogs.model.Dialog.Toolbar.Operation> operationList) {
        JsonArray result = new JsonArray();
        operationList.forEach(operation -> result.add(jsonOperationOf(operation)));
        return result;
    }

    public static JsonObject jsonOperationOf(io.intino.konos.server.activity.displays.dialogs.model.Dialog.Toolbar.Operation operation) {
        JsonObject result = new JsonObject();
