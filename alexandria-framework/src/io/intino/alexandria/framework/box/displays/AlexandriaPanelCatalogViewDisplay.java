...
import io.intino.alexandria.framework.box.displays.notifiers.AlexandriaPanelCatalogViewDisplayNotifier;
...
public class AlexandriaPanelCatalogViewDisplay extends AlexandriaPanelViewDisplay<AlexandriaPanelCatalogViewDisplayNotifier> {
	...
	private void createCatalogDisplay() {
		...
		buildDisplay(catalog).ifPresent(display -> {
			display.filter(item -> render.filter(context(), target(), (io.intino.alexandria.framework.box.model.Item) item));
