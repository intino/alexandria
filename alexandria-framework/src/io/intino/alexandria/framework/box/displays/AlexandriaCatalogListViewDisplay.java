...
import io.intino.alexandria.framework.box.displays.builders.CatalogSortingBuilder;
...
import io.intino.alexandria.framework.box.displays.builders.PictureDataBuilder;
import io.intino.alexandria.framework.box.displays.notifiers.AlexandriaCatalogListViewDisplayNotifier;
...
import io.intino.alexandria.framework.box.model.catalog.events.OpenPanel;
import io.intino.alexandria.framework.box.model.mold.Stamp;
import io.intino.alexandria.framework.box.model.mold.stamps.Picture;
import io.intino.alexandria.framework.box.model.mold.stamps.Title;
...
public class AlexandriaCatalogListViewDisplay extends PageDisplay<AlexandriaCatalogListViewDisplayNotifier> implements AlexandriaCatalogViewDisplay {
	...
	private void renderDisplays(String item) {
		Map<io.intino.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> displays = displays(item);
	...
	private void refreshPictures(String itemId, List<Picture> pictures) {
		io.intino.alexandria.framework.box.model.Item item = provider.item(new String(Base64.getDecoder().decode(itemId)));
	...
	protected void init() {
		...
		List<io.intino.alexandria.framework.box.model.catalog.arrangement.Sorting> sortings = provider.sortings();
	...
	private void sendSortingList(List<io.intino.alexandria.framework.box.model.catalog.arrangement.Sorting> sortings) {
	...
	private Map<io.intino.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> displays(String record) {
		List<Stamp> stamps = provider.stamps(view.mold()).stream().filter(s -> (s instanceof io.intino.alexandria.framework.box.model.mold.stamps.Display)).collect(toList());
		Map<io.intino.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> nullableMap = stamps.stream().collect(Collectors.toMap(s -> (io.intino.alexandria.framework.box.model.mold.stamps.Display)s, stamp -> provider.display(stamp.name())));
		Map<io.intino.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> result = nullableMap.entrySet().stream().filter(e -> e.getValue() != null).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
	...
	private ElementViewDisplayProvider.Sorting sortingOf(io.intino.alexandria.framework.box.model.catalog.arrangement.Sorting sorting) {
	...
	private ElementViewDisplayProvider.Sorting sortingOf(String name, String mode) {
		return new ElementViewDisplayProvider.Sorting() {
			...
			@Override
			public int comparator(io.intino.alexandria.framework.box.model.Item item1, io.intino.alexandria.framework.box.model.Item item2) {
	...
	private List<io.intino.alexandria.framework.box.model.Item> selectedItems() {
