...
import io.intino.konos.alexandria.foundation.activity.displays.PageDisplay;
import io.intino.konos.alexandria.foundation.activity.spark.ActivityFile;
import io.intino.konos.alexandria.framework.box.displays.builders.CatalogSortingBuilder;
import io.intino.konos.alexandria.framework.box.displays.builders.ElementViewBuilder;
import io.intino.konos.alexandria.framework.box.displays.builders.ItemBuilder;
import io.intino.konos.alexandria.framework.box.displays.builders.PictureDataBuilder;
import io.intino.konos.alexandria.framework.box.displays.notifiers.AlexandriaCatalogListViewDisplayNotifier;
...
import io.intino.konos.alexandria.framework.box.model.catalog.events.OpenPanel;
import io.intino.konos.alexandria.framework.box.model.mold.Stamp;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.Picture;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.Title;
import io.intino.konos.alexandria.framework.box.model.mold.stamps.Tree;
...
import static io.intino.konos.alexandria.framework.box.helpers.ElementHelper.*;
...
public class AlexandriaCatalogListViewDisplay extends PageDisplay<AlexandriaCatalogListViewDisplayNotifier> implements AlexandriaCatalogViewDisplay {
	...
	private void notifyOpenItem(String item) {
		openItemListeners.forEach(l -> l.accept(new OpenItemEvent() {
			...
			@Override
			public io.intino.konos.alexandria.framework.box.model.Item item() {
	...
	private void renderDisplays(String item) {
		Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> displays = displays(item);
	...
	private void refreshPictures(String itemId, List<Picture> pictures) {
		io.intino.konos.alexandria.framework.box.model.Item item = provider.item(new String(Base64.getDecoder().decode(itemId)));
	...
	protected void init() {
		...
		List<io.intino.konos.alexandria.framework.box.model.catalog.arrangement.Sorting> sortings = provider.sortings();
	...
	private void sendSortingList(List<io.intino.konos.alexandria.framework.box.model.catalog.arrangement.Sorting> sortings) {
	...
	private Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> displays(String record) {
		List<Stamp> stamps = provider.stamps(view.mold()).stream().filter(s -> (s instanceof io.intino.konos.alexandria.framework.box.model.mold.stamps.Display)).collect(toList());
		Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> nullableMap = stamps.stream().collect(Collectors.toMap(s -> (io.intino.konos.alexandria.framework.box.model.mold.stamps.Display)s, stamp -> provider.display(stamp.name())));
		Map<io.intino.konos.alexandria.framework.box.model.mold.stamps.Display, AlexandriaStampDisplay> result = nullableMap.entrySet().stream().filter(e -> e.getValue() != null).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
	...
	private ElementViewDisplayProvider.Sorting sortingOf(io.intino.konos.alexandria.framework.box.model.catalog.arrangement.Sorting sorting) {
	...
	private ElementViewDisplayProvider.Sorting sortingOf(String name, String mode) {
		return new ElementViewDisplayProvider.Sorting() {
			...
			@Override
			public int comparator(io.intino.konos.alexandria.framework.box.model.Item item1, io.intino.konos.alexandria.framework.box.model.Item item2) {
	...
	private List<io.intino.konos.alexandria.framework.box.model.Item> selectedItems() {
