...
import io.intino.konos.alexandria.ui.model.mold.stamps.Picture.AvatarProperties;
import io.intino.konos.alexandria.ui.model.mold.stamps.icons.ResourceIcon;
import io.intino.konos.alexandria.ui.model.mold.stamps.operations.OpenExternalDialogOperation;
import io.intino.konos.alexandria.ui.model.mold.stamps.operations.PreviewOperation;
...
public class ItemBuilder {
    ...
    public static List<Item> buildListOnlyLocation(List<io.intino.konos.alexandria.ui.model.Item> itemList, ItemBuilderProvider provider, URL baseAssetUrl) {
        return itemList.stream().map(item -> ItemBuilder.buildOnlyLocation(item, provider, baseAssetUrl)).collect(toList());
    }

    private static String label(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider) {
    ...
    private static Instant group(io.intino.konos.alexandria.ui.model.Item item, TimeScale scale) {
        if (item == null) return null;
        Instant created = item.created();
        return created != null ? scale.normalise(created) : null;
    }

    private static List<ItemBlock> itemBlockList(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
        return provider.blocks().stream().map(block -> itemBlock(item, provider, baseAssetUrl, block)).collect(toList());
    }

    private static ItemBlock itemBlock(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl, Block block) {
    ...
    private static List<ItemStamp> itemStampList(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
        return provider.stamps().stream().map(stamp -> recordItemStamp(item, provider, baseAssetUrl, stamp)).collect(toList());
    }

    private static List<ItemStamp> itemLocationStampList(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
    ...
    private static ItemStamp recordItemStamp(io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl, Stamp stamp) {
    ...
    private static List<String> valuesOf(Stamp stamp, io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
    ...
    private static String valueOf(Stamp stamp, io.intino.konos.alexandria.ui.model.Item item, Object value, ItemBuilderProvider provider, URL baseAssetUrl) {
    ...
    private static List<Property> propertiesOf(Stamp stamp, io.intino.konos.alexandria.ui.model.Item item, ItemBuilderProvider provider, URL baseAssetUrl) {
