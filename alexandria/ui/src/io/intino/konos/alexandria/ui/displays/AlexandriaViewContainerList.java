...
public class AlexandriaViewContainerList extends AlexandriaViewContainerCollectionPage<AlexandriaViewContainerListNotifier> implements AlexandriaStampProvider {
	...
	private void renderDisplays(String item) {
		...
		io.intino.konos.alexandria.ui.model.Item itemObject = itemOf(item);
	...
	private void sendSortingList(List<io.intino.konos.alexandria.ui.model.catalog.arrangement.Sorting> sortings) {
		notifier.refreshSortingList(CatalogSortingBuilder.buildList(sortings.stream().filter(io.intino.konos.alexandria.ui.model.catalog.arrangement.Sorting::visible).collect(toList())));
	}
	...
	private ElementViewDisplayProvider.Sorting sortingOf(String name, String mode) {
		return new ElementViewDisplayProvider.Sorting() {
			...
			@Override
			public int comparator(io.intino.konos.alexandria.ui.model.Item item1, io.intino.konos.alexandria.ui.model.Item item2) {
