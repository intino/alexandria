...
public class AlexandriaCatalogListViewDisplay extends PageDisplay<AlexandriaCatalogListViewDisplayNotifier> implements AlexandriaCatalogViewDisplay {
	...
	private void renderDisplays(String item) {
		Map<Display, AlexandriaStampDisplay> displays = displays(item);
	...
	private Map<Display, AlexandriaStampDisplay> displays(String record) {
		List<Stamp> stamps = provider.stamps(view.mold()).stream().filter(s -> (s instanceof Display)).collect(toList());
		Map<Display, AlexandriaStampDisplay> nullableMap = stamps.stream().collect(Collectors.toMap(s -> (Display)s, stamp -> provider.display(stamp.name())));
		Map<Display, AlexandriaStampDisplay> result = nullableMap.entrySet().stream().filter(e -> e.getValue() != null).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
