...
import io.intino.alexandria.foundation.activity.dialogs.builders.DialogBuilder;
import io.intino.alexandria.foundation.activity.dialogs.builders.ValidationBuilder;
...
public abstract class DialogDisplay extends Display<DialogDisplayNotifier> {
	private Box box;
	...
	public io.intino.alexandria.foundation.activity.spark.ActivityFile downloadResource(String path) {
		io.intino.alexandria.foundation.activity.schemas.Resource resource = dialog.input(path).value().asResource();

		return new io.intino.alexandria.foundation.activity.spark.ActivityFile() {
			@Override
			public String label() {
	...
	private DialogValidator.Result validateResource(FormInput formInput) {
		...
		List<io.intino.alexandria.foundation.activity.schemas.Resource> resourceValues = dialog.input(formInput.path()).values().asResource();
		Map<String, byte[]> valuesMap = resourceValues.stream().collect(toMap(io.intino.alexandria.foundation.activity.schemas.Resource::name, o -> Base64.decodeBase64(o.value())));
